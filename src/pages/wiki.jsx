...
/* ---------------- External data source ----------------
   data.json lives in a SEPARATE repo (e.g. yourname/mia-wiki-data) so a
   collaborator can be given access to just that repo and edit the file
   (and photos in photos/) in-browser on github.com, with no access to
   this site's repo.

   Served via raw.githubusercontent.com, which is fronted by GitHub's own
   CDN and picks up new commits quickly (usually near-instant to a couple
   minutes) — no manual cache purge needed after an edit. jsDelivr is
   available as a commented-out alternative below if you'd rather trade
   that for slightly more robust CORS/uptime; it just requires manually
   purging its cache after each update (see comment near DATA_BASE). */

const DATA_REPO = "ttom795/MarineWiki"; // <-- change this
const DATA_BRANCH = "main";

// Base URL for everything in the data repo (data.json AND the photos/ folder).
// raw.githubusercontent.com (recommended: fronted by GitHub's own CDN, which
// picks up new commits fast — usually near-instant to a couple minutes,
// no manual cache purge needed. Good fit for a wiki people actively edit.)
const DATA_BASE = `https://raw.githubusercontent.com/${DATA_REPO}/${DATA_BRANCH}`;

// Alternative: jsDelivr (slightly more robust CORS/uptime, but its CDN can
// cache a file for up to ~7 days after a commit; you'd need to hit
// https://purge.jsdelivr.net/gh/${DATA_REPO}@${DATA_BRANCH}/<path> after
// each update to force a refresh)
// const DATA_BASE = `https://cdn.jsdelivr.net/gh/${DATA_REPO}@${DATA_BRANCH}`;

// data.json fetch gets its own cache-busting query param so edits show up
// on every reload rather than being served from the browser's cache.
const DATA_URL = `${DATA_BASE}/data.json?t=${Date.now()}`;
...
function EntityPhoto({ id, name, type }) {
  const [src, setSrc] = useState(`${DATA_BASE}/photos/${id}.png?t=${Date.now()}`);
    ...
    <img
      ...
      onError={() => {
        if (!triedFallback.current) {
          ...
          setSrc(`${DATA_BASE}/photos/blank.png?t=${Date.now()}`);
        }
      }}
    />
  );
    ...
    <>
      ...
      <div
        style={{
          ...
          transitionDuration: e.id === "char-ornigol" ? "2s" : "0.2s",
      ...
      <div className={`dossier ${typeClass}`}>
        ...
        {e.type === "character" && (
          <div className="photo-frame" style={{ transform: `rotate(${Math.random() * 12 - 6}deg)` }}>
            <EntityPhoto key={e.id} id={e.id} name={e.name} type={e.type} />
          </div>
        )}
        </div>
...
.mia-wiki .dossier-header { display:flex !important; gap:24px !important; align-items:flex-start !important; margin-bottom:4px !important; }
.mia-wiki .dossier-header-info { flex:1 1 auto !important; min-width:0 !important; }
.mia-wiki .photo-frame { position: relative; margin-bottom:18px; top:20px;
  box-shadow:
    0 4px 10px rgba(0, 0, 0, 0.25),
    0 1px 2px rgba(0, 0, 0, 0.15) !important;flex: 0 0 auto !important; width: 140px !important; height: 180px !important; padding: 8px 8px 22px 8px !important; background: #fdfdf8 !important; border: 1px solid #d8d5cb !important; border-radius: 2px !important;}
.mia-wiki .dossier-photo { display:block !important; width:100% !important; height:100% !important; object-fit:cover !important; filter:
    grayscale(100%); transform: translateZ(0);}
...
.photo-frame::after {
  ...
  background:
    ...
    radial-gradient(circle at 90% 90%, rgba(0,0,0,0.25), transparent 20%);

  mix-blend-mode: overlay;
  opacity: .8;
}
.mia-wiki .dossier h1 { font-family:Georgia, 'Palatino Linotype', 'Book Antiqua', Palatino, serif !important; font-size:2.3rem !important; margin:0 0 4px 0 !important; letter-spacing:0.3px !important; max-width:100% !important; }
.mia-wiki .aliases { font-style:italic !important; color:var(--muted) !important; font-size:0.8rem !important; margin-bottom:18px !important; }
...
.mia-wiki .badge-row { display:flex !important; gap:2px !important; flex-direction: column; margin-bottom:12px !important; }
...
.mia-wiki .dossier-header { padding-top:0 !important; }
}
